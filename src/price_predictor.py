...
import matplotlib
matplotlib.use('Agg')  # Non-interactive backend for our grand plot
...
# Prepare for the model battle
print("Gearing up the model...")
X = data[['Days']].values
...
print("Model victorious! Score:", model.score(X_test, y_test))

# Predict the future harvest
print("Peering into the future...")
last_day = X[-1][0]
future_day = last_day + 30
future_price = model.predict([[future_day]])
print(f"Predicted maize price in 30 days: KES {future_price[0]:.2f}")
...
try:
    ...
    plt.figure(figsize=(12, 8))  # Bigger canvas for drama
